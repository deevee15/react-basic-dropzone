import { useRef, useState, DragEvent, ChangeEvent, SetStateAction, Dispatch } from 'react'
...
const Dropzone = ({ accept, maxFiles, disabled, maxFileSize, textContent, upload }: DropzoneProps) => {
    ...
    return (
        <>
            <div className="mt-6" aria-label="Dropzone element" aria-describedby="dropzone-title" aria-disabled={disabled}>
                <div>
                    <input 
                        type="file" 
                        ref={fileInputRef} 
                    ...
                    <div 
                        ...
                        className={`
                            ...
                            ${disabled 
                                ...
                                : 'cursor-pointer hover:bg-green-50 active:bg-green-200'}
                            ${dragOver && !disabled ? 'bg-green-50' : ''}
                        `}
                    >
                        <DropzoneIcon />
                        <p className="text-xl text-center font-medium text-green-800" id='dropzone-title'>
                            {textContent?.title || `Drag and drop files here or click to select`}
                        </p>
                        <p className='text-xs' id='dropzone-description'>
                            {textContent?.description || `Max file size: ${formatSize(maxFileSize)}`}
                        </p>
                    </div>
                </div>
                            ...
                            processedFiles.length > 0 ?
                                processedFiles.map((oneFile, index) => {
                                    return (
                                        <FileElement 
                                            file={oneFile.file}
                                            uploadingAllowed={oneFile.uploaded ? false : true}
                                            setProcessedFiles={setProcessedFiles as Dispatch<SetStateAction<FileWithId[]>>}
                                            uploadFunc={upload}
                                            deleteFunc={() => removeFile(oneFile.id)} 
                                            key={oneFile.id}
                                        />
                                    )
                                })
